...
std::vector<Seed>& SeedBank::getSeedElements() {
    return seedElements;
}
...
void SeedBank::foreachAllSeed(std::function<void(Seed&)> call, CRuntimeClass* thePlant) {
    for (auto& seed : seedElements) {
...
void SeedBank::draw(HDC hDC) {
    ...
    // dock[0]->AlphaBlend(hDC, leftX + SLIDING_WINDOW, topY, dock[0]->GetWidth() * 1.25, dock[0]->GetHeight() * 1.25, 0, 0, dock[0]->GetWidth(), dock[0]->GetHeight());
    for (auto& seed : seedElements) {
        seed.draw(hDC);
    }
    // 显示阳光量
    ...
    CRect textArea(leftX, topY + (int)(height * 0.7), leftX + (int)(width * 0.18), topY + height);
    ::DrawText(hDC, sunlightStr, sunlightStr.GetLength(), &textArea, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
...
int SeedBank::getSunlight() {
    return sunlight;
}

void SeedBank::setSunlight(int s) {
    sunlight = s;
}